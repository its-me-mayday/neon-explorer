...
export class Player {
  constructor(scene, planets, environment) {
    ...
    this.yawSpeed = 2.0;
    this.rollSpeed = 2.8;
    this.baseAcceleration = 80;
    this.turboAcceleration = 240;
  ...
  buildShip() {
    ...
    const cockpit = new THREE.Mesh(new THREE.SphereGeometry(0.35, 16, 12, 0, Math.PI * 2, 0, Math.PI / 2), cockpitMat);
    cockpit.position.set(0, 0.2, 0.4);
    cockpit.scale.set(1, 0.6, 1.5);
    this.shipParts.add(cockpit);

    this.cameraAnchor = new THREE.Object3D();
    this.cameraAnchor.position.set(0, 0.4, 1.5);
    ...
    this.enginePositions.forEach(pos => {
      ...
      eGroup.position.set(pos.x, pos.y, pos.z);
      eGroup.rotation.x = Math.PI / 2;
  ...
  toggleCamera() {
    ...
    this.enginesGroup.visible = !isFirst;
  ...
  startWarp() {
    ...
    this.planets.forEach(p => {
      const diff = p.mesh.position.z - this.mesh.position.z;
  ...
  update(delta) {
    ...
    if (this.keys.level) {
      const levelQ = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, this.mesh.rotation.y, 0));
      this.mesh.quaternion.slerp(levelQ, 0.1);
    }
    ...
    if (this.isTurbo) {
      ...
      this.turboEnergy = Math.min(100, this.turboEnergy + 25 * delta);
    }
    if (this.turboFill) this.turboFill.style.width = `${this.turboEnergy}%`;

    const direction = new THREE.Vector3(0, 0, 1).applyQuaternion(this.mesh.quaternion);
    const accel = this.isTurbo ? this.turboAcceleration : this.baseAcceleration;
    if (this.keys.forward) this.velocity.addScaledVector(direction, accel * delta);
    if (this.keys.backward) this.velocity.addScaledVector(direction, -accel * delta);
    if (this.isCruise && this.velocity.length() < 50) {
    ...
    if (this.speedDisplay) {
      this.speedDisplay.innerText = Math.round(this.velocity.length() * 10).toString().padStart(3, '0');
    }

    const engineScale = (this.isTurbo ? 2.5 : 1) + Math.sin(Date.now() * 0.005) * 0.1;
    const engineColor = this.isTurbo ? 0xffffff : 0x00ffff;
    this.engineCores.forEach(core => {
      core.scale.setScalar(engineScale);
      core.material.color.setHex(engineColor);
    });
  }
  ...
  checkCollisions() {
    ...
    for (const asteroid of this.asteroids) {
      if (pos.distanceTo(asteroid.position) < 15 + asteroid.scale.x * 0.5) {
        this.handleCollision(asteroid.position);
        return;
      }
    }
    for (const p of this.planets) {
      if (pos.distanceTo(p.mesh.position) < p.radius + 15) {
        this.handleCollision(p.mesh.position);
        return;
      }
    }
  }
  ...
  handleCollision(objectPos) {
    this.velocity.copy(this.mesh.position.clone().sub(objectPos).normalize().multiplyScalar(50));
  ...
  updateRibbonTrails() {
    this.enginePositions.forEach((pos, i) => {
      ...
      if (pts.length > 1) {
        ...
        m.lookAt(pts[pts.length - 1]);
        m.scale.z = pts[0].distanceTo(pts[pts.length - 1]) || 0.1;
        m.material.opacity = this.isTurbo ? 0.8 : 0.4;
      }
    });
  }
  ...
  updateRadar() {
    ...
    this.planets.forEach(p => {
      const rel = p.mesh.position.clone().sub(this.mesh.position);
      if (rel.length() >= 50000) return;
      const dot = document.createElement('div');
      dot.className = 'radar-dot';
      dot.style.cssText = `position:absolute;left:${(rel.x / 50000) * 75 + 75}px;top:${(rel.z / 50000) * 75 + 75}px;width:5px;height:5px;background:#00ffff;border-radius:50%;box-shadow:0 0 8px #00ffff;`;
